...
abstract class OpenSID {
	...
	public static function run() {
		...
		if ( version_compare( str_replace( '-src', '', $wp_version ), '4.3', '<' ) ) {
			if ( current_user_can( 'update_plugins' ) ) {
				add_action( 'admin_notices', array( 'OpenSID', 'show_minimum_requirements_error_notice' ) );
			}
			return;
		}
	...
	public static function load_view( $view, array $data = array() ) {
		...
		$ucview = ucfirst( $view );
		$the_view = self::load_class( "OpenSID_{$ucview}_View", "view-{$view}.php", 'views' );
